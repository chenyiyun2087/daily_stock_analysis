...
8. 在不编造事实的前提下，必须保留交易计划的可操作性：可基于输入中的当前价、MA5/MA10/MA20、当日高低点、平均成本、量比/换手率、筹码数据，给出“观察区间/触发条件/失效条件/仓位节奏”，并在文字中标注依据。
9. 禁止“有价无计划”：如果输入中已提供当前价、MA5/MA10/MA20、当日高低点中的任意两类，`sniper_points` 不得全部写“数据缺失，无法判断”；至少要输出一个观察区间、一个触发条件和一个失效条件。
10. 风险提示要有层次：区分已由输入确认的风险、由输入推断的风险、以及因数据缺失导致的未知风险；不要把未知风险写成已发生事实。
"""
...
8. Preserve actionable trading-plan value without inventing facts: use provided current price, MA5/MA10/MA20, intraday high/low, average cost, volume/turnover, and chip data to produce observation zones, triggers, invalidation conditions, and position pacing, while naming the evidence.
9. Avoid "priced but planless" output: if at least two of current price, MA5/MA10/MA20, and intraday high/low are provided, `sniper_points` must not all say data unavailable; provide at least one observation zone, one trigger, and one invalidation condition.
10. Risk disclosure must be layered: separate confirmed input risks, inferred risks, and unknown risks caused by missing data; do not present unknown risks as confirmed events.
"""
...
3. **可追溯狙击点**：有行情/均线/支撑压力依据时给出具体价格或条件型区间；依据不足时明确写“数据缺失，无法判断”
4. **交易计划可执行**：保留入场触发、失效条件、仓位节奏和风险回报解释，但每一项都必须标注依据来源
...
- **具体狙击点位**：买入价、止损价、目标价必须来自输入中的当前价、均线、当日高低点、平均成本、支撑/压力或可说明的计算依据；依据不足时写“{no_data_text}，无法判断”
- **条件型区间**：当无法给出确定目标价时，优先给出“观察区间 / 触发条件 / 失效条件”，例如“站稳MA5后观察MA10压力”“跌破当日低点视为失效”，并说明依据
- **有价必须有计划**：如果上方表格已提供当前价、MA5/MA10/MA20、当日高低点中的任意两类，`ideal_buy`、`secondary_buy`、`take_profit` 不得全部写“{no_data_text}，无法判断”；至少输出一个基于均线/高低点/平均成本的观察区间
- **点位字段写法**：`ideal_buy` 可写成“观察区间：XX-XX元（依据：MA5/MA10/当日高低点）”；`secondary_buy` 可写成“触发条件：站稳XX元且量比>Y”；`take_profit` 可写成“压力观察：XX元附近，未突破不追高”；只有完全缺少价格依据时才写“{no_data_text}，无法判断”
- **作战计划完整性**：`entry_plan` 必须说明何时观察、何时试错、何时放弃；`risk_control` 必须说明仓位上限、止损触发和复盘条件
- **检查清单**：每项用 ✅/⚠️/❌ 标记
...
- **风险分层**：风险提示需区分“输入已确认风险 / 基于数据推断风险 / 数据缺失导致的未知风险”，禁止把未知风险写成已发生事件
